...
- **Do NOT make Jot itself a multi-user backend.** Assign + sync + permissions + presence = rebuilding Jira. Aidin already has Jira in the ecosystem (the Atlassian MCP, Skiff). For a team project, let Jira be the source of truth - assignment + sync come for free from it.
...
- If team: whose team (the employer?), and does it need permissions or is "everyone with the link can edit" enough?
